...
<details>
  ...
  <ol>
    ...
    <li>
      ...
      <ul>
        <li><a href="#hub--console-backend--admin-dashboard">Hub + Console (Backend + Admin Dashboard)</a></li>
        <li><a href="#kiosk-android-app">Kiosk (Android App)</a></li>
        <li><a href="#reskiosk-relay-nodes-lora">ResKiosk RELAY Nodes (LoRa)</a></li>
      </ul>
    </li>
...
*ResKiosk* is a complete offline-first voice-powered kiosk system designed for disaster shelters, evacuation centers, and humanitarian aid sites. It enables displaced individuals to ask questions by voice in their native language and receive spoken answers about medical services, registration, sleeping arrangements, transportation, safety, and more — all without requiring internet connectivity.

The system consists of three core components:

| Component | Description |
|-----------|-------------|
| *Hub + Console* | Unified Python/FastAPI backend and React admin console for semantic search, translation, LLM response processing, KB operations, emergency operations, and logs |
| *Kiosk* | Android tablet app with offline speech-to-text, text-to-speech, and emergency detection |
| *ResKiosk RELAY Nodes* | LoRa relay nodes for resilient inter-hub communication and message relay |

All AI models run locally. The hub, kiosks, console, and RELAY nodes can communicate over LAN/LoRa without cloud dependency.

<p align="right">(<a href="#readme-top">back to top</a>)</p>

### Key Features

- *Fully Offline Voice Pipeline* — Speak in 5 languages (and more on the way), get answers spoken back. No internet needed.
...
- *LLM-Powered Response Formatting and Query Enrichment (Two Models)*
  - *Formatter model* for user-facing response rewriting.
  - *Rewriter model* for noisy/unclear query rewriting in fallback paths.
- *Real-Time Emergency Alerts* — Voice keyword detection, SOS button, tiered alert lifecycle, SSE broadcast to console.
- *RLHF-Style Retrieval Bias* — Thumbs up/down feedback shifts article ranking over time (configurable).
- *Multilingual Translation* — Facebook NLLB-200 translates queries and responses between supported languages.
...
- *Sherpa-ONNX* — Offline STT (Zipformer for en, Whisper for ja/es/de/fr) and TTS (VITS for en/es/de/fr, Android System TTS for ja)
...
- *Ollama* — Local LLMs (split formatter + rewriter)
- *SQLAlchemy* — ORM for KB, config, logs, and emergency data
- *React Router / Axios / Lucide React* — Console UI stack
- *PyInstaller* — Standalone .exe packaging

*ResKiosk RELAY Nodes*

- *LoRa transport integration* via hub relay routes and serial manager
- *Optional AES-256-GCM encryption* for relay messages
...
| *Disk* | 15 GB+ free | Model weights + build artifacts |
...
# 3. Start Hub + Console
.\TO RUN\03_start_hub_console.bat
...
2. Build and deploy to an Android tablet.
3. Complete in-app setup to download required offline models.
...
### Hub + Console (Backend + Admin Dashboard)

This combined component handles:

- *Query Processing* — Receives transcripts from kiosks, translates, normalizes, retrieves answers, rewrites/format with split LLM models, translates back.
- *Knowledge Base* — SQLite-backed articles with categories/tags/embeddings and full admin CRUD.
- *Shelter Configuration* — Structured shelter sections with freshness enforcement.
- *Emergency Management* — Receives kiosk alerts, broadcasts via SSE, tracks lifecycle (Active → Acknowledged → Responding → Resolved).
- *Session + Logging* — Per-kiosk history, query logs with intent/score/latency, feedback storage.
- *RELAY Integration* — LoRa routes, message relay, encryption support.
...
- *Voice Interaction* — Tap-to-speak with ring buffer pre-capture and final transcript output after decode (no live transcript preview).
...
- *Emergency Detection* — Tier 1 and Tier 2 detection plus manual SOS hold-to-confirm.
...
- *Two Chat Modes* — Voice and Text (switchable in-session).

### ResKiosk RELAY Nodes (LoRa)

RELAY nodes provide low-bandwidth inter-hub relay transport for message delivery where internet/LAN paths are constrained.
...
The full speech-to-response pipeline runs offline-first:
...
User speaks → Audio capture (ring buffer) → STT decode → Post-processing → Emergency check
    ...
    → MiniLM embed → Cosine similarity search → Gating → LLM rewriter (fallback) → LLM formatter → NLLB translate back
...
| *STT* | SherpaSttEngine | Zipformer (en), Whisper (ja/es/de/fr); transcript displayed after decode |
| *Post-Processing* | SttPostProcessor.kt | Dedup, fillers, time normalization, fuzzy domain corrections, punctuation |
| *Translation* | translator.py (NLLB-200) | Bidirectional: user language → EN, EN → user language |
| *Normalization* | normalizer.py | Lowercase, dedup, domain corrections, synonym mappings |
| *Intent Classification* | intent.py | Prototype-based intents, cosine centroid scoring, top-2 enrichment |
| *Semantic Search* | search.py | MiniLM-L6-v2 embeddings, cosine similarity, top-k retrieval |
| *Query Rewrite* | rewriter.py | Dedicated rewriter LLM for unclear/noisy queries |
| *Response Formatting* | formatter.py | Dedicated formatter LLM for response phrasing |
| *TTS* | SherpaTtsEngine | VITS (en/es/de/fr) + Android System TTS (ja) |
...
1. *Direct Config Match* — Exact key lookup in structured config.
2. *Intent Classification* — Prototype embeddings per intent, cosine similarity to centroids.
3. *Short-Circuit* — Greeting, identity, capability, small_talk, goodbye return static responses.
4. *Query Enrichment* — Intent keywords appended to search query (supports dual-intent enrichment).
5. *Vector Search* — MiniLM-L6-v2 embedding → cosine similarity against KB article matrix.
6. *RLHF Bias* — Optional score adjustment from historical feedback.
7. *Gating* — Confidence-based DIRECT_MATCH / NEEDS_CLARIFICATION / NO_MATCH.

### 23+ Supported Intents

greeting · identity · capability · small_talk · food · medical · registration · sleeping · transportation · safety · facilities · lost_person · pets · donations · hours · location · general_info · goodbye · mental_health · legal_docs · financial_aid · hygiene · departure · children · special_needs
...
ResKiosk includes a complete emergency alert lifecycle across components.
...
- *Manual SOS* — Hold-to-confirm button (3-second press)
...
Operators can activate shelter-wide emergency mode from the Dashboard. Kiosks receive state via polling and display emergency UI states with alarm audio.
...
3. *Bias Rebuild* — Periodic offline job converts feedback into per-article bias scores.
4. *Retrieval Adjustment* — adjusted_score = raw_cosine + (RLHF_ALPHA * bias).
...
| Japanese | ja | Batch | Whisper | Yes |
| Spanish | es | Batch | Whisper | No (better raw) |
| German | de | Batch | Whisper | No (better raw) |
| French | fr | Batch | Whisper | No (better raw) |

- *Transcript display*: all languages show final transcript after recording/decode (no live preview).
- *STT engines*: English uses Zipformer; Japanese/Spanish/German/French use Whisper.
- *Translation* uses NLLB-200-distilled-600M for query→EN and EN→response language.
- *Hub normalization* includes language-specific synonym mappings.

All KB content is authored in English. Non-English queries are translated to English for search, and responses are translated back to the user's language.
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | /lora/status | Relay device connection status |
| POST | /lora/send | Send relay message |
| POST | /lora/send_ack | Send relay acknowledgment |
| GET | /lora/ws/lora | Relay websocket stream |
...
| *Zipformer* (en) | Streaming STT | ~50 MB | Kiosk models |
| *Whisper* (ja/es/de/fr) | Batch STT | ~150 MB each | Kiosk models |
| *VITS* (en/es/de/fr) | TTS | ~20 MB each | Kiosk models |
| *Android System TTS* (ja) | TTS | device-dependent | Android voice data |
...
|-------------|:--------:|------------------|-------------|
| OPENAI_API_KEY | Optional/future | [Get from OpenAI](https://platform.openai.com/api-keys) | Reserved for future cloud integration (currently paused) |
